...
//fetchLinkLists();
  ...
  if(_college==''){
    ...
    // });
      ...
      loadColleges_table($('#search_college_state :selected').val(),$('#inst_type :selected').val(),$('#inst_courses :selected').val());
      ...
      $('body').on('change','#inst_type',function(){
        ...
        loadColleges_table(state,type,stream,0);
      });

      $('body').on('change','#inst_courses',function(){
        var state=$('#search_college_state :selected').val();
        var type=$('#inst_type :selected').val();
        var stream=$('#inst_courses :selected').val();
      ...
      function loadColleges_table(state,inst_type,inst_courses,created_by){
        var college_list_table=$('#college_list_table').DataTable({ 
          'bJQueryUI': false,
          'stateSave': true,
          'iDisplayLength':75,
          ...
          "lengthMenu": [[10,25,50,75,100,250,500,1000,1500], [10,25,50,75,100,250,500,1000,1500]],
          "processing": true, //Feature control the processing indicator.
          "serverSide": true, //Feature control DataTables' server-side processing mode.
          "order": [], //Initial no order.
          // Load data for the table's content from an Ajax source
          "ajax": {
              "url": base_url+'/institutions/colleges/search',
              "type": "POST",
              "data":{csrf_test_name:csrf_hash,state_id:state,inst_type:inst_type,inst_courses:inst_courses,created_by:created_by}                 
          },
    ...
    $('body').on('change','#search_college_state',function(){
      ...
      $('#college_list_table').DataTable().destroy();
      loadColleges_table(state,type,stream);
    });
  ...
  if(_college!=''){
    ...
    $('body').on('change','#college_gallery_category',function(){
      var v=this.value;
      if(v=='7'){
        $('#collegeInnerMenuesGalleryModal').find('div#college_gallery_image_div').css({'display':'none'});
        $('#collegeInnerMenuesGalleryModal').find('div#college_gallery_video_div').css({'display':'block'});
      }else{
        $('#collegeInnerMenuesGalleryModal').find('div#college_gallery_image_div').css({'display':'block'});
        $('#collegeInnerMenuesGalleryModal').find('div#college_gallery_video_div').css({'display':'none'});
      }
    });
  ...
  $('#form_college').validate({
    ...
    submitHandler:function(){
      $.ajax({
          ...
          data:new FormData($('#form_college')[0]),
          cache: false,
          contentType: false,
          processData: false,
          timeout: 60000000,
    ...
    Swal.fire({
      ...
      if (result.value){
        $.ajax({
          ...
          data:{_college:_college,csrf_test_name:csrf_hash},
          beforeSend:function(){
  ...
  $('body').on('click','#btn_add_cutoff',function(){
    ...
    cutoffhtml+='<tr style="background-color: #00000017;border-top:1px #0000000f solid;" id="tr'+rowcutoff+'">';
      cutoffhtml+='<td style="width:20%;">';
        cutoffhtml+='<label class="control-label">Exams</label>';
        cutoffhtml+='<select class="form-control" name="college_cutoff['+rowcutoff+'][exams]">'+cutoff_exams+'</select>';
      cutoffhtml+='</td>';
      cutoffhtml+='<td style="width:10%;">';
        cutoffhtml+='<label class="control-label">Year</label>';
        cutoffhtml+='<select class="form-control" name="college_cutoff['+rowcutoff+'][year]">'+cutoff_year+'</select>';
      cutoffhtml+='</td>';
      cutoffhtml+='<td style="width:10%;">';
        cutoffhtml+='<label class="control-label">Rounds</label>';
        cutoffhtml+='<select class="form-control" name="college_cutoff['+rowcutoff+'][rounds]">'+cutoff_round+'</select>';
      cutoffhtml+='</td>';
      cutoffhtml+='<td style="width:60%;">';
        cutoffhtml+='<label class="control-label">Courses</label>';
        cutoffhtml+='<select class="form-control" name="college_cutoff['+rowcutoff+'][courses]">'+cutoff_course+'</select>';
      cutoffhtml+='</td>';
    cutoffhtml+='</tr>';
    cutoffhtml+='<tr style="background-color: #00000017;border-top:1px #0000000f solid;" id="tr_'+rowcutoff+'">';
      cutoffhtml+='<td style="width:20%;">';
        cutoffhtml+='<label class="control-label">Categories</label>';
        cutoffhtml+='<select class="form-control" name="college_cutoff['+rowcutoff+'][categories]">'+sub_categories+'</select>';
      cutoffhtml+='</td>';
      cutoffhtml+='<td style="width:40%;">';
        cutoffhtml+='<label class="control-label">Sub Categories</label>';
        cutoffhtml+='<select class="form-control" name="college_cutoff['+rowcutoff+'][sub_categories]">'+sub_categories+'</select>';
      cutoffhtml+='</td>';
      cutoffhtml+='<td style="width:20%;">';
        cutoffhtml+='<label class="control-label">Cutoff Value</label>';
...
$( "#collegeBroucherUpdateModal" ).on('shown.bs.modal', function(){
 ...
 var crankr=colleg_ranking_row;
    ...
    $(document).on('click','#btn_add_college_ranking_row',function(){
      ...
      rhtml+='<td>';
      rhtml+='<select class="form-control" name="college_ranking['+crankr+'][ques]">';
 ...
 var cfr=colleg_faq_row;
    ...
    $(document).on('click','#btn_add_college_faqus_row',function(){
      ...
      html+='</td>';
      html+='<td>';
      html+='<textarea class="form-control" name="college_faqus['+cfr+'][ans]" aria-describedby="college_faqus" placeholder="Answer" rows="5"></textarea>';
      html+='</td>';
      html+='<td>';
    ...
    $(document).on('click','#btn_add_college_placement_faqus_row',function(){
      ...
      html+='</td>';
      html+='<td>';
      html+='<textarea class="form-control" name="college_placement_faqs['+cfr2+'][ans]" aria-describedby="college_faqus" placeholder="Answer" rows="5"></textarea>';
      html+='</td>';
      html+='<td>';
    ...
    $(document).on('click','#btn_add_college_scholarship_faqus_row',function(){
      ...
      html+='</td>';
      html+='<td>';
      html+='<textarea class="form-control" name="college_scholarship_faqs['+cfr3+'][ans]" aria-describedby="college_faqus" placeholder="Answer" rows="5"></textarea>';
      html+='</td>';
      html+='<td>';
      html+='<button type="button" class="btn btn-sm btn-danger" onclick="$(\'#tr3' + cfr3 + '\').remove()"><i class="fa fa-minus"></i></button>';
      html+='</td>';
      html+='</tr>';
