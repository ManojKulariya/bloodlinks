...
<div class="container">
 <div class="timeline">
	        ...
	        <div class="row mb-2">
	          ...
	          <div class="col-sm-6">
	            <ol class="breadcrumb float-sm-right">
					...
					<div onclick="selectTimeRange(10)">Last 15 Days</div>
					...
					<input type="hidden" name="daysfillter" id="daysfillter" value="15">
   ...
   <div class="card-headers">
                        <b>Pending Request For Blood</b>&nbsp;
                        <a id="downloadpending_req_BL_excelLink" href="#"
                           style="text-decoration: underline; color: #0D2942;">Download Excel</a>
                     </div>
                     <!-- /.card-header --> 
                     <div class="card-bodys">
                        <table id="table_bb_request" class="table table-bordered table-hover">
                           <thead>
                              <tr>
                                 <th>#</th>
                                 <th>Blood Bank</th>
                        ...
                        <a href="#" id="load_more_link_bb_req">View More</a>
                     </div>
...
var bb_request_excel = '<?php echo $base_url; ?>/bb_request_excel';
//------------------------------------------
//var days_filter = 0;
...
$(document).ready(function () {
  var tablereq = $('#table_bb_request').DataTable({
        ...
        "initComplete": function (settings, json) {
            $('#table_bb_request_paginate').hide();
        },
        "drawCallback": function (settings) {
            toggleLoadLinkbloodreq();
            if (tablereq.page.info().pages > 1) {
                $('#table_bb_request_paginate ').show();
            } else {
                $('#table_bb_request_paginate ').hide();
            }
        }
    ...
    function toggleLoadLinkbloodreq() {
        var totalRecords3 = tablereq.page.info().recordsTotal;
        var currentLength3 = tablereq.page.len();
        var link = $('#load_more_link_bb_req');
        if (currentLength3 < totalRecords3) {
            link.text('View More');
        } else {
            link.text('View Less');
        }
    }
    $('#load_more_link_bb_req').on('click', function (event) {
        event.preventDefault();
        var currentLength3 = tablereq.page.len();
        var totalRecords3 = tablereq.page.info().recordsTotal;
    
        if (currentLength3 < totalRecords3) {
            tablereq.page.len(currentLength3 + 3).draw();
        } else {
            tablereq.page.len(3).draw();
        }
        toggleLoadLinkbloodreq();
    });
    toggleLoadLinkbloodreq();
