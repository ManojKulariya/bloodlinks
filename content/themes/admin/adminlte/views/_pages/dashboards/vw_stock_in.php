            ...
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-2">
                            <div class="form-group">
                                <label for="blood_group">Request To</label>
                    ...
                    <div id="bloodBankDetailTable" style="display: none;">
                      ...
                      <form id="bloodDonationForm" action="<?php echo $base_url;?>/bb_stock_transfer_in_save" method="POST">
                      <input type="hidden" name="by" id="by" />
                        <div class="row" style="margin-bottom:4px;">
                            <div class="col-md-1">
                                <div class="form-group">
                                  <label for="blood_group">Blood Group</label>
                                    <select name="blood_group[]" required id="blood_group" class="form-control">
                                        <option  selected="selected" value="">Select</option>
                            ...
                            <div class="col-md-2">
                              <div class="form-group">
                                  <label for="collection_date">Donation Date</label>
                                  <input type="date" class="form-control collection_date" required id="collection_date" name="collection_date[]">
                              </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label for="expiry_date">EXP. Date</label>
                                    <input type="date" class="form-control expiry_date" readonly required id="expiry_date" name="expiry_date[]">
                                </div>
                            </div>
                            <div class="col-md-1">
                                <div class="form-group">
                                    <label for="tti_test">Old Unit No</label>
                                    <input type="text" class="form-control old_unit_no" required id="old_unit_no" name="old_unit_no[]">
                                    <span class="error-message"></span>
                                 </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label for="tti_test">Unit No</label>
                                    <input type="text" class="form-control unit_no" required id="unit_no" name="unit_no[]">
                                    <span class="error-message"></span>
                                 </div>
                            </div>
                            <div class="col-md-1">
                                <div class="form-group">
                                    <label for="blood_volume">Quantity</label>
                                    <input type="number" class="form-control" required id="blood_volume" name="blood_volume[]">
                                </div>
                            </div>
                            <div class="col-md-1">
                                <div class="form-group">
                                    <label for="tti_test">TTI Test</label>
                                    <select class="form-control" required id="tti_test" name="tti_test[]">
                            ...
                            <div class="col-md-1">
                                <div class="form-group">
                                    <label for="tti_test">Tube No</label>
                                    <input type="text" class="form-control" required id="tube" name="tube[]">
                                </div>
                            </div>
                            <div class="col-md-1">
                              <div class="form-group">
                                <button type="button" style="margin-top:20px;" class="btn btn-primary circle-btn add-row">+</button>
                              </div>
                            </div>
                        </div> 
                          <div class="form-group">
...
$(document).ready(function() {
    ...
    function addRow(afterElement) {
        ...
        <div class="row" style="margin-bottom:2px;">
            <div class="col-md-1">
                <div class="form-group">
                    <select name="blood_group[]" required class="form-control blood_group">
                        <option selected="selected"  value="">Select</option>
                        <option >A+</option>
                        <option >A-</option>
                        <option >B+</option>
            ...
            <div class="col-md-2">
                <div class="form-group">
                    <input type="date" class="form-control collection_date" required name="collection_date[]">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <input type="date" class="form-control expiry_date" readonly required name="expiry_date[]">
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
            ...
            <div class="col-md-1">
                <div class="form-group">
                    <input type="number" required class="form-control blood_volume" name="blood_volume[]">
                </div>
            </div>
            <div class="col-md-1">
                <div class="form-group">
            ...
            <div class="col-md-1">
                <div class="form-group">
                    <button type="button" class="btn btn-danger circle-btn remove-row">-</button>
                </div>
            </div>
        </div>`;
        $(afterElement).after(newRow);
        // $('#bloodDonationForm').append(newRow);
    }
    ...
    function validateUnitNos() {
    var unitNos = [];
    var isValid = true;
    $('.unit_no').each(function() {
        var unitNo = $(this).val().toUpperCase(); // Convert to uppercase
        var errorMessageSpan = $(this).siblings('.error-message');
        if (unitNo) {
            if (unitNos.includes(unitNo)) {
                errorMessageSpan.text('Unit No must be unique');
                isValid = false;
            } else {
                unitNos.push(unitNo);
            ...
            errorMessageSpan.text('Unit No is required');
            isValid = false;
        }
    });
    ...
    $(document).on('change', '.collection_date', function() {
            var donationDate = new Date($(this).val());
            if (!isNaN(donationDate.getTime())) {
                var expirationDate = new Date(donationDate);
                expirationDate.setDate(expirationDate.getDate() + <?php echo $exp_days;?>); // Add 48 days
                var formattedExpirationDate = expirationDate.toISOString().split('T')[0];
                $(this).closest('.row').find('.expiry_date').val(formattedExpirationDate);
            }
        });
    $(document).on('keyup', '.unit_no', function() {
            var currentInput = $(this);
            var unitNo = currentInput.val();
            var errorMessageSpan = currentInput.siblings('.error-message');
            validateUnitNos();
            var url2 = '<?php echo $base_url; ?>/unit_validation';
            if (unitNo) {
                $.ajax({
                    url: url2,
                    method: 'POST',
                    ...
                    success: function(response) {
                        if (response.status === 'exists') {
                            errorMessageSpan.text('Unit No already exists');
