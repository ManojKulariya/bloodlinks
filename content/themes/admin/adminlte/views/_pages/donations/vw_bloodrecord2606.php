      ...
      <tbody>
        ...
        if (!empty($_POST)) {
          ...
          if (!empty($blood_type) && !empty($blood_group) && !empty($Component) && !empty($unit_no) && !empty($tti_test) && !empty($cross_match) && !empty($issue)) {
            ...
            $query = $this->db->query("SELECT * FROM bl_blood_record  WHERE bl_blood_record.bloodbank_id = '$bank_id' And $search ORDER BY (CASE WHEN expiry_date < CURDATE() THEN 1 ELSE 0 END), expiry_date ASC");
          }
        } else {
