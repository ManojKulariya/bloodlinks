   ...
   <div class="row" style="box-shadow: 0 0 1px rgb(0 0 0 / 13%), 0 1px 3px rgb(0 0 0 / 20%); border-radius: 2px; width: 100%; margin-bottom: 25px; padding: 8px;">
      ...
      <div class="col-md-3">
         <div class="form-group">
            ...
            <input type="text" class="form-control" id="price" name="reason" value="<?php if(isset($_POST) && isset($_POST['reason'])){ echo $_POST['reason']; } ?>">
         </div>
      </div>
      <div class="col-md-3">
                           <div class="form-group">
        ...
        <div class="col-md-3">
           ...
           {
              ?>
          <option value="<?= $type->district_name; ?>" <?php if(isset($_POST) && isset($_POST['city'])){  if($_POST['city'] == $type->district_name) {echo"selected='selected'";} } ?>><?= $type->district_name; ?></option>
            <?php } ?>
                                </select>
            </div>
         </div>
...
<div class="container" style="
   ...
   <table class="table table-fluid table-responsive" id="myTable">
      <thead>
         <tr>
            <th class="th-width">S No.</th>
            <th class="th-width">Appointment Id</th>
            <th class="th-width">Donar Name</th>
            <th class="th-width">Blood Group</th>
            <th class="th-width">Mobile No</th>
            <th class="th-width">Date</th>
            <th class="th-width">Reason</th>
            <?php 
      ...
      <tbody>
        ...
        if(!empty($_POST)){
          ...
          if (!empty($app_id) && !empty($reason) && !empty($blood_group) && !empty($city)  && (!empty($start_date) && !empty($end_date))) {
            ...
            $query = $this->db->query("SELECT bl_blood_request.*,bl_customers.first_name,bl_customers.mid_name,bl_customers.last_name,bl_customers.blood_group,bl_customers.ph_no , bl_districts.district_name FROM bl_blood_request INNER JOIN bl_customers ON bl_customers.user_id = bl_blood_request.user_id INNER JOIN bl_districts ON bl_districts.id = bl_customers.district_id WHERE requested_schedule_date BETWEEN '$start_date' AND '$end_date' AND bl_blood_request.org_id = '$bank_id' AND bl_blood_request.approved_status = 'Reject' And bl_blood_request.application_no = '$app_id' And bl_blood_request.reject_reason = '$reason' And bl_districts.district_name = '$city' And bl_blood_request.blood_group = '$blood_group' ORDER BY ID DESC");
            ...
            if(!empty( $search)) {
              ...
              $search .="bl_blood_request.application_no  LIKE '%$app_id%'";
            }
			if(!empty($reason)){
            if(!empty( $search)) {
						 $search .=' And ';
					}
              $search .= "bl_blood_request.reject_reason  LIKE '%$reason%'";
            }
            ...
            if(!empty( $search)) {
              ...
              $search .= "bl_districts.district_name LIKE '%$city%'";     
            }
            ...
            $query = $this->db->query("SELECT bl_blood_request.*,bl_customers.first_name,bl_customers.mid_name,bl_customers.last_name,bl_customers.blood_group,bl_customers.ph_no , bl_districts.district_name FROM bl_blood_request INNER JOIN bl_customers ON bl_customers.user_id = bl_blood_request.user_id INNER JOIN bl_districts ON bl_districts.id = bl_customers.district_id WHERE bl_blood_request.org_id = '$bank_id' And bl_blood_request.approved_status = 'Reject' $querySearch ORDER BY ID DESC");

          }
            }else{
            $query = $this->db->query("SELECT bl_blood_request.*,bl_customers.first_name,bl_customers.mid_name,bl_customers.last_name,bl_customers.blood_group,bl_customers.ph_no , bl_districts.district_name FROM bl_blood_request INNER JOIN bl_customers ON bl_customers.user_id = bl_blood_request.user_id INNER JOIN bl_districts ON bl_districts.id = bl_customers.district_id WHERE bl_blood_request.org_id = '$bank_id' And bl_blood_request.approved_status = 'Reject' ORDER BY ID DESC"); 
            }
             foreach ($query->result() as $row) {
        ...
        $row[]  ='<button type="button" class="btn btn-xs btn-dark" onclick="deleteFun('.$row->id.');" ><i class="fa fa-trash"></i></button>';
   }else{
        $row[]  = '';
      }
    }else{
...
<div class="row">
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">Deferred Request</h3>
      </div>

      <div class="card-body">
        <table id="table_requestdeferred_donor" class="table table-bordered table-hover">
          <thead>
          <tr>
            <th>#</th>
  ...
  <script type="text/javascript">
    var apppointment_search='<?php echo $base_url;?>/donations/onSearchdeferred_request';
...
// alert(id);
    ...
    { 
      ...
      $.ajax({
               ...
               url:'<?php echo $base_url;?>/donations/deferred_request_delete',
               method:"POST",
               datatype:"json",
