...
$userid= $this->uri->segment(5);
 $query1 = $this->db->query("SELECT * FROM bl_customers WHERE user_id = '$userid'");
 foreach ($query1->result() as $row1)
{

}
if ($row1->gender == 'male') {
// redirect('admin/donations/donation_form3/'.$id.'/'.$userid.'');
$query = $this->db->query("SELECT * FROM bl_donar_form_info WHERE form_id = '$id' AND form_step = 'step_2'");
}else{
$query = $this->db->query("SELECT * FROM bl_donar_form_info WHERE form_id = '$id' AND form_step = 'step_3'");
...
<form id="form_step_3" class="msform">
   <fieldset>
                     ...
                     <div class="pre-question">
                        <div class="buttons">
                           <ol>
                              ...
                              <li><b>Danger:</b>the window period,laboratory tests are negative but the person is still capable of infecting others. Help keep the blood supply as safe as possible by lookingHONESTLY at your lifestyle & answering the question truthfully.</li>
                           </ol>
                        </div>
                     </div>
                  </div>
      ...
      <a href="<?php echo base_url('admin/donations/donation_form4/'.$id.'/'.$userid.'');?>"  class="btn btn-success btn-md pull-right" type="button"/>Next</a>
     <!--  <input type="submit" name="next" class="next action-button" id="next_form" data-form_type="step_<?php echo $step;?>" value="Next" />
      <input type="submit" name="back" class="back action-button" id="next_form" data-form_type="step_<?php echo $step_back;?>" value="Back"/> -->
   </fieldset>
   <!--3rd step tab end-->
</form>
