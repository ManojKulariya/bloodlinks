...
class Landlord extends CI_Controller
{
	...
	function tenants($param1 = '', $param2 = '', $param3 = '')
	{
		if (!$this->session->userdata('user_type'))
			redirect(base_url() . 'login', 'refresh');
		...
		if (in_array($this->db->get_where('module', array('module_name' => 'tenants'))->row()->module_id, $this->session->userdata('permissions'))) {
			...
			$page_data['name'] 	= 	$param1;
			$page_data['value'] = 	$param2;
			$page_data['value2'] = 	$param3;
...
// attendance start
	...
	function expenses($param1 = '', $param2 = '',$param3 = '')
	{
	   // echo $param1 .'111'. "<br/>";
	   // echo $param2 .'222'. "<br/>"; die;
		if (!$this->session->userdata('user_type'))
			redirect(base_url() . 'login', 'refresh');

		if (in_array($this->db->get_where('module', array('module_name' => 'expenses'))->row()->module_id, $this->session->userdata('permissions'))) {
			
			if ($param1 == 'staff') $this->model->staff_expense();
			elseif ($param1 == 'update') $this->model->update_expense($param2);
         	elseif ($param1 == 'edit') $this->model->update_staff_expense($param2);
        //  	elseif ($param1 == 'name') $this->model->getAllDataBetweenDates($param2);
...
// Inventory Start 
	...
	function add_inventory()
	{
		if (!$this->session->userdata('user_type'))
			...
			$page_data['page_name'] 	= 	'add_inventory';
			$this->load->view('index', $page_data);
			}

	function inventory($param1 = '', $param2 = '', $param3 = '')
	{

			if ($param1 == 'add') $this->model->add_inventory();
 			elseif ($param1 == 'update') $this->model->update_inventory($param2);
 			elseif ($param1 == 'update1') $this->model->update_inventory1($param2);
        	elseif ($param1 == 'remove') $this->model->remove_inventory($param2);
        	elseif ($param1 == 'edit') $this->model->edit_inventory($param2);
	...
	function add_category()
	{
			...
			$page_data['page_title']	=	'Inventory Category';
	...
	function category($param1 = '', $param2 = '')
	{
			...
			if ($param1 == 'add') $this->model->add_category();
 			elseif ($param1 == 'update') $this->model->update_category($param2);
 			elseif ($param1 == 'update1') $this->model->update_category1($param2);
        	elseif ($param1 == 'remove') $this->model->remove_category($param2);
    ...
    function add_sub_category()
	{
			...
			$page_data['page_title']	=	'Inventory Category';
			$page_data['page_name'] 	= 	'add_sub_category';
			$this->load->view('index', $page_data);
	}
	function sub_category($param1 = '', $param2 = '')
	{

			if ($param1 == 'sub') $this->model->sub_category();
            elseif ($param1 == 'edit') $this->model->update_sub_category($param2);
            elseif ($param1 == 'remove') $this->model->remove_sub_category($param2);
			$page_data['navbar_status']	=	'aside-collapsed';
			$page_data['page_title']	=	'Category';
			$page_data['page_name'] 	= 	'sub_category';
			$this->load->view('sub_category', $page_data);
...
// Services 
      function service_invoice()
	{
			...
			$this->load->view('service_invoice', $page_data);
	}
	function add_service_invoice($param1 = '')
	{
		if ($param1 == 'add') $this->model->service_invoice();
          	$page_data['page_title']	=	'Service Invoice';
			$page_data['page_name'] 	= 	'add_service_invoice';
			$this->load->view('add_service_invoice', $page_data);
	}
function generate_service_invoice($param = '')
	{
	
			$page_data['tenant_id']	=	$param;
			$page_data['page_title']	=	'generate_service_invoice';
			$page_data['page_name'] 	=	'generate_service_invoice';
			$this->load->view('generate_service_invoice', $page_data);
...
// End Service Invoice
	...
	function board_members($param1 = '', $param2 = '',$param3 = '')
	{
		if (!$this->session->userdata('user_type'))
			redirect(base_url() . 'login', 'refresh');
...
// 	**Forget Password **
	...
	{
		if ($param1 == 'update') $this->model->forget_password($param2);
	...
	function single_year_rents_report($year = '',$month = '')
	{
		if (!$this->session->userdata('user_type'))
			redirect(base_url() . 'login', 'refresh');
