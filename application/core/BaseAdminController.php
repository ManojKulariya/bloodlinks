...
class BaseAdminController extends CI_Controller
{
    ...
    function __construct(){
        ...
        //print_obj($this->active_controller);die;
        ...
        if(session_userdata('isAdminLoggedin')==TRUE && session_userdata('admin_id')){
            ...
            //         'Organisations'=>array(
            //             'Hospitals'=>$this->data['base_url'].'/hospitals',
            ...
            if($user_type=='1'){
                ...
                $name=$userdata->short_name;
            }else if($user_type=='0'){
                $name='blood bank user';
            }
    ...
    public function onUploadFiles($param,$uploader_method='uploader'){
     ...
     public function wp_test($group,$name,$bank_id){
        ...
        // untested $query8 = $this->db->query("SELECT * FROM bl_bb_donatioform where status = '' AND blood_group ='O-' AND bloodbank_id = $bank");
        // tested $query9 = $this->db->query("SELECT * FROM bl_bb_donatioform where wb = '' AND blood_group ='A+' AND bloodbank_id = $bank
        if($name == 'Tested'){
          $txt = 'Test Done'; 
        }else{
          $txt = 'Test Not Done';  
